...
// Create context
const PinTestContext = createContext<any>(null);

// Provider component
export const PinTestProvider = ({ children }: { children: React.ReactNode }) => {
  const { user } = useAuth();
  ...
  return (
    <PinTestContext.Provider value={contextValue}>
      {children}
    </PinTestContext.Provider>
  );
};

// Hook to use the context
export const usePinTest = () => {
  const context = useContext(PinTestContext);
  if (!context) {
    throw new Error('usePinTest must be used within a PinTestProvider');
  }
  return context;
};